...
class Trainer:
    def __init__(self, options):
        ...
        self.model_lr_scheduler = optim.lr_scheduler.MultiStepLR(self.model_optimizer, milestones=self.opt.milestones, gamma=0.1)
    ...
    def create_models(self):
        ...
        if self.depth_input == 'fusion':
            ...
            #self.models["encoder"] = networks.DeepNet('mpvitnet')
        ...
        #self.models["depth"] = networks.DepthDecoder2()
        ...
        if self.use_posenet:
            if self.pose_input == 'fusion':
                ...
                self.models["pose_encoder"] = networks.VIT(input_size=(self.opt.height,self.opt.width), num_input_channels=6)
                self.models["pose"] = networks.PoseDecoder(num_ch_enc=np.array([384]), num_input_features=1, num_frames_to_predict_for=2)
    ...
    def process_batch(self, inputs):
        ...
        for scale in self.opt.scales:
            ...
            #disp = outputs[("disp", 0, scale)].detach()
    ...
    def predict_poses(self, inputs, depth):
        ...
        if self.pose_input == "rgb":
            ...
            for f_i in self.opt.novel_frame_ids[1:]:
                if f_i < 0:
                    pose_inputs = torch.cat([pose_feats[f_i], pose_feats[0]], 1)
                else:
                    pose_inputs = torch.cat([pose_feats[0], pose_feats[f_i]], 1)

                pose_inputs, attn_map = self.models["pose_encoder"](pose_inputs)
                #pose_inputs, attn_map = self.models["pose_encoder"](torch.cat([pose_inputs, preprocess_disp(depth)], 1))
            ...
            for f_i in self.opt.novel_frame_ids[1:]:
                ...
                '''
                outputs[("query", 0, f_i)] = query
                outputs[("key", 0, f_i)] = key
                outputs[("value", 0, f_i)] = value
                '''
            ...
            for frame_id in self.opt.novel_frame_ids:
                ...
                if frame_id != 0:
                    ...
                    '''writer.add_image("similarity_map_{}/{}".format(frame_id, self.epoch),
                             self.show_attn_on_image(inputs["color", 0, 0][j], outputs[("similarity", 0, frame_id)][j]).transpose(2,0,1), val_idx+j)'''
        ...
        with torch.no_grad():
            for inputs in dataloader:
                ...
                if self.pose_input == "rgb":
                    ...
                    #pose_inputs = torch.cat([all_color_aug, preprocess_depth(depth)], dim=1)
                    pose_inputs, attn_map = self.models["pose_encoder"](all_color_aug)
                    axisangle, translation = self.models["pose"](pose_inputs)
                    ...
                    outputs = self.models['depth'](features)
                    _, depth = disp_to_depth(outputs[("disp", 0, 0)], self.opt.min_depth, self.opt.max_depth)
                    #_, depth = disp_to_depth(outputs[("disp", 0, 0)].detach(), self.opt.min_depth, self.opt.max_depth)
