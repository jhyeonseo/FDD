...
class VIT(nn.Module):
    def __init__(self, model_type='vit_base_patch16_384', pretrained=True, num_classes=0, input_size=(192,640), patch_size=[16,16], start_index=1, num_input_channels=7):
    ...
    def make_pose(self, input):
        ...
        for idx in chunks:
            ...
            median = torch.median(part, dim=-1)[0].unsqueeze(-1).cuda()        # B, 1, 1
            mask_index = (part > median).cuda()                                # B, 1, num_indices
            attn[:,:,idx] = part
            mask = part.clone()
            mask[mask_index] *= 0
            masked[:,:,idx] = mask
